...
public class UserDao {
	...
	public User login(User u) throws Exception {
		...
		if (con == null) {
			...
			try {
				...
				user.setPanel(rs.getString("panel"));
	...
	public boolean addUser(User u) {
		u.setPanel(RoleHelper.getIndex(u.getRole()));
		JDBCUtil jdbcUtil = new JDBCUtil();
		try {
			Connection con = jdbcUtil.getCon();
			Statement stmt = con.createStatement();
			String sql = "insert into " + usertable + "(name,pwd,real,dep,role,panel) values('" + u.getUsername()
					+ "', " + u.getPassword() + ", N'" + u.getReal() + "', N'" + u.getDep() + "', N'" + u.getRole()
					+ "', N'" + u.getPanel() + "')";
	...
	public boolean updateUser(User u) {
		...
		try {
			Connection con = jdbcUtil.getCon();
			String sql = "update " + usertable + " set pwd='" + u.getPassword() + "', real=N'" + u.getReal()
					+ "', dep=N'" + u.getDep() + "', role=N'" + u.getRole() + "' where name='" + u.getUsername() + "'";
			Statement stmt = con.createStatement();
			int ok = stmt.executeUpdate(sql);
			if (ok > 0) {
				return true;
	...
	public boolean deleteUser(User u) {
		...
		try {
			Connection con = jdbcUtil.getCon();
			String sql = "delete from " + usertable + " where name='" + u.getUsername() + "'";
			Statement stmt = con.createStatement();
			int ok = stmt.executeUpdate(sql);
			if (ok > 0) {
				return true;
	...
	public String search(String page, String keyword) {
		...
		try {
			...
			if (con != null) {
				...
				String sql = "SELECT TOP 5 * FROM " + usertable + keyword + isWhere + "  ID NOT IN(SELECT TOP "
						+ ((pageNo - 1) * 5) + " ID FROM " + usertable + keyword + " ORDER BY ID) ORDER BY ID";
	...
	public String getRealName(String username) {
		...
		try {
			...
			if (con != null) {
				Statement stmt = con.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
				String sql = "SELECT real FROM " + usertable + "where name='" + username + "';";
				ResultSet rs = stmt.executeQuery(sql);
				if (rs.next()) {
					realName = rs.getString("real").trim();
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
